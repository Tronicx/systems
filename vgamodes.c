...
#define VGA_FRAMEBUFFER_ADDR (0x0a0000)
...
int VGA_SetGraphicsMode(uint16_t width, uint16_t height, uint8_t chain4Mode) 
{
	...
	bpp = 8;
	chain4 = chain4Mode;
	frameBufferPtr = (void *)VGA_FRAMEBUFFER_ADDR;
	frameBufferSize = (width * height * bpp) >> 3;
...
int VGA_GetGraphicsMode(GraphicsMode * modePtr)
{
	...
	modePtr->Width = screenWidth;
	modePtr->Height = screenHeight;
	modePtr->Bpp = bpp;
	modePtr->Chain4 = chain4;
	modePtr->FrameBufferPtr = frameBufferPtr;
	modePtr->FrameBufferSize = frameBufferSize;
